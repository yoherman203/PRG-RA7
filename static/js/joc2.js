...
zonaJoc.addEventListener("mousemove", (e) => {
	...
	if (posicioX > AMPLADA_JOC - AMPLADA_COTXE)
		posicioX = AMPLADA_JOC - AMPLADA_COTXE;
...
function finalitzarJoc() {
	...
	fetch("/finalitzar_joc", {
		...
		.then((response) => {
			if (response.ok) {
				console.log(
					"Dades enviades correctament al servidor per a ser guardades.",
				);
			}
		})
		.catch((error) => {
			console.error(
				"No s'han pogut enviar les dades al servidor:",
				error,
			);
		});
}
